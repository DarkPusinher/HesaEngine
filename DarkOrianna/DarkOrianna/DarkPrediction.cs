...
namespace DarkOrianna
{
    public static class DarkPrediction
    {
        ...
        public static List<Vector3> dir = new List<Vector3>();
        public static List<Vector3> dirC = new List<Vector3>();
        ...
        public static Vector3 LinearPrediction(Vector3 skillInitialPosition, Spell spell, AIHeroClient enemy)
        {
            ...
            if (enemy != null)
            {
                if (!enemy.IsDead)
                {
                    ...
                    if (epos.Count > 1)
                    {
                        if (epos[epos.Count - 1] == epos[epos.Count - 2])
                        {
                            return enemy.Position;
                        }
        ...
        public static Vector3 BestCastPosition(Spell spell, int targetHit, int radiusSpell) // Finds Best Cast position of a skill
        {
            ...
            var enemies = ObjectManager.Heroes.Enemies.Where(enemy => enemy != null & enemy.IsValidTarget(spell.Range));
            int count = 0;
        ...
        public static Vector3 CirclerPrediction(Spell spell, AIHeroClient enemy, int hitChance) // hitChance 0 = medium, 1 = high
        {
            var player = ObjectManager.Me;
            ...
            if (enemy != null)
            {
                if (!enemy.IsDead)
                {
                    ...
                    if (eposC.Count > 0)
                    {
                        if (eposC[eposC.Count - 1] == eposC[eposC.Count - 2])
                        {
                            stoperC = false;
                            return enemy.Position;
                        }
                    ...
                    if (hitChance == 1)
                    {
                        if (eposC.Count > 3 && stoperC == false)
                        {
                            if (((eposC[eposC.Count - 2] - eposC[eposC.Count - 3]).Normalized()) != ((eposC[eposC.Count - 3] - eposC[eposC.Count - 4]).Normalized()))
                            {
                                Vector3 loc1 = (enemy.Position + ((eposC[eposC.Count - 2] - eposC[eposC.Count - 3]).Normalized())) * 1000;
                                Vector3 loc2 = (enemy.Position + ((eposC[eposC.Count - 3] - eposC[eposC.Count - 4]).Normalized())) * 1000;
                                if (loc1.Distance(loc2) > 10)
                                {
                                    stoperC = true;
                                    delayer2 = Game.GameTimeTickCount + 300;
                                }
                                else
                                {
                                    return empt;
                        ...
                        if (stoperC = true && Game.GameTimeTickCount <= delayer2)
                        {
